...
public class ElementActions {
        ...
        waitManager.fluentWait().until(d ->
                {
                    try {
                        ...
                        LogsManager.info("Condition not yet met for " + locator + ": " + e.getClass().getSimpleName() + " - " + e.getMessage());
    ...
    public ElementActions hoverAndClick(By hoverLocator, By clickLocator) {
        waitManager.fluentWait().until(d -> {
            try {
                WebElement hoverEl = d.findElement(hoverLocator);
                scrollToElementJS(hoverLocator);
                WebElement clickEl = d.findElement(clickLocator);
                new Actions(d)
                        .moveToElement(hoverEl)
                        .moveToElement(clickEl)
                        .click()
                        .perform();
                LogsManager.info("Hovered and clicked: " + clickLocator);
                return true;
            } catch (Exception e) {
                LogsManager.info("hoverAndClick failed: " + e.getClass().getSimpleName() + " - " + e.getMessage());
                return false;
            }
        });
        return this;
